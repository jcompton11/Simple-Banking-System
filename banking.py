...
class BankSys:
    def __init__(self, bank_card, database_nm='card.s3db'):
        ...
        # Create a new table "card" to store bank card and user info
        self.cursor = self.d_conn.cursor()

        # Check if table "card" already exists
    ...
    # this function executes repetitive queries where the database is changed
    ...
    # This function tests the card number we will be using to make transfers
    def test_card(self, card_num):
        # sql query to check if card exists in database before updating
        sql_query = f"SELECT COUNT(number) FROM card WHERE number = {card_num};"
        self.cursor.execute(sql_query)
        res = self.cursor.fetchone()[0]

        # first test card number using Luhn algorithm
    ...
    # This function makes the transfer to a different card
    def transfer(self, amount):
        # Test if the balance is right
        self.get_balance()
